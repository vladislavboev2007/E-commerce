...
class GiftWrapDecorator(ProductDecorator):
    def __init__(self, product: ProductComponent, wrap_cost: Decimal = Decimal('199.00')):
    ...
    def get_description(self) -> str:
        return f"{self._product.get_description()} + Подарочная упаковка ({self.wrap_cost}₽)"


class ExpressShippingDecorator(ProductDecorator):
    def __init__(self, product: ProductComponent, shipping_cost: Decimal = Decimal('499.00')):
        super().__init__(product)
    ...
    def get_description(self) -> str:
        return f"{self._product.get_description()} + Срочная доставка ({self.shipping_cost}₽)"


class PersonalizationDecorator(ProductDecorator):
    def __init__(self, product: ProductComponent, personalization_text: str,
                 personalization_cost: Decimal = Decimal('299.00')):
        super().__init__(product)
    ...
    def get_description(self) -> str:
        return f"{self._product.get_description()} + Персонализация: '{self.personalization_text}' ({self.personalization_cost}₽)"


class InsuranceDecorator(ProductDecorator):
    def __init__(self, product: ProductComponent, insurance_cost: Decimal = Decimal('249.00')):
        super().__init__(product)
    ...
    def get_description(self) -> str:
        return f"{self._product.get_description()} + Страхование товара ({self.insurance_cost}₽)"


class ExtendedWarrantyDecorator(ProductDecorator):
    def __init__(self, product: ProductComponent, warranty_cost: Decimal = Decimal('999.00')):
        super().__init__(product)
        self.warranty_cost = warranty_cost

    def get_price(self) -> Decimal:
        return self._product.get_price() + self.warranty_cost

    def get_description(self) -> str:
        return f"{self._product.get_description()} + Расширенная гарантия ({self.warranty_cost}₽)"


class GreetingCardDecorator(ProductDecorator):
    def __init__(self, product: ProductComponent, card_cost: Decimal = Decimal('99.00')):
        super().__init__(product)
        self.card_cost = card_cost

    def get_price(self) -> Decimal:
        return self._product.get_price() + self.card_cost

    def get_description(self) -> str:
        return f"{self._product.get_description()} + Поздравительная открытка ({self.card_cost}₽)"


class DecoratorManager:
    def __init__(self):
        self.decorators = {
            "Подарочная упаковка": GiftWrapDecorator,
            "Срочная доставка": ExpressShippingDecorator,
            "Персонализация (гравировка)": PersonalizationDecorator,
            "Страхование товара": InsuranceDecorator,
            "Расширенная гарантия": ExtendedWarrantyDecorator,
            "Поздравительная открытка": GreetingCardDecorator
        }
    ...
    def apply_decorators(self, base_product: BaseProduct, decorator_types: List[str], **kwargs) -> ProductComponent:
        ...
        for decorator_type in decorator_types:
            if decorator_type in self.decorators:
                if decorator_type == "Персонализация (гравировка)":
